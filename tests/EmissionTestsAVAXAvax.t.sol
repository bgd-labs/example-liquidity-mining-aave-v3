...
contract EmissionTestsAVAXAvax is BaseTest {
  ...
  uint256 constant TOTAL_DISTRIBUTION = 300 ether; // 300 wAVAX/15 Days
  ...
  function test_activation() public {
    ...
    vm.startPrank(sAVAX_A_TOKEN_WHALE);

    vm.warp(block.timestamp + 15 days);

    address[] memory assets = new address[](1);
    assets[0] = sAVAX_A_TOKEN;

    uint256 balanceBefore = IERC20(REWARD_ASSET).balanceOf(sAVAX_A_TOKEN_WHALE);
    ...
    uint256 balanceAfter = IERC20(REWARD_ASSET).balanceOf(sAVAX_A_TOKEN_WHALE);

    uint256 deviationAccepted = 300 ether; // Approx estimated rewards with current emission in 1 month
    assertApproxEqAbs(
      balanceBefore,
      balanceAfter,
      deviationAccepted,
